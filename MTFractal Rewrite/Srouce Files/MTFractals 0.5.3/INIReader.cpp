...
{
	#if __DBG && __INI && __LOG
	std::ofstream mainINIHandLog;
	mainINIHandLog.open("mainINIHand.log.txt");
	#endif
...
{
	#if __DBG && (__INI||__FRACTAL||__RANDOM) && __LOG
	std::ofstream fracINIHandLog;
	fracINIHandLog.open("fracINIHand.log.txt");
	#endif
	...
	{
		#if __DBG && (__INI||__FRACTAL||__RANDOM) && __LOG
		fracINIHandLog.close();
		#endif
		return 0;  /* unknown section/name, error */
	}
	#if __DBG && (__INI||__FRACTAL||__RANDOM)
	fracINIHandLog.close();
	#endif
	return 1;
}
...
{
	#if __DBG && (__INI||__FRACTAL||__COLOR)  && __LOG
	std::ofstream setINIHandLog;
	setINIHandLog.open("setINIHand.log.txt");
	#endif
