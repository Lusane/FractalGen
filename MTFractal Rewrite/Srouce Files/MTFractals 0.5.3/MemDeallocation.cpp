...
void RTA_Deallocate() 
{
	...
	std::ofstream RTADLog;
	RTADLog.open("RTA_D.log.txt");
	#endif
...
void TVA_Deallocate() 
{
	...
	std::ofstream TVADLog;
	TVADLog.open("TVA_D.log.txt");
	#endif
...
void RemoveFrac() 
{
	#if __DBG && (__MEMORY||__FRACTAL) && __LOG
	std::ofstream RmFracLog;
	RmFracLog.open("RmFrac.log.txt");
	#endif
	slp(10);
