...
{
	#if __DBG && __FRACTAL && __LOG
	std::ofstream DefaultsLog;
	DefaultsLog.open("Defaults.log.txt");
	#endif
...
{
	#if __DBG && (__FRACTAL||__RANDOM) && __LOG
	std::ofstream RandFracValLog;
	RandFracValLog.open("RandFracVal.log.txt");
	#endif
	...
	#if __DBG && (__FRACTAL||__RANDOM) && __LOG
	RandFracValLog.close();
	#endif
}
void randomColors() 
{
	#if __DBG && (__COLOR||__RANDOM) && __LOG
	std::ofstream RandColorValLog;
	RandColorValLog.open("RandColorVal.log.txt");
	#endif
...
void genFrac(int InitX, int FinalX, int InitY, int FinalY, int id)
{
	std::stringstream ID;
	ID << id;
	#if (__DBG && (__FRACTAL||__COLOR||_THREAD) && __LOG) || __TESTING
	std::ofstream FracGenLog;
	std::string logName = "FracGen_Thread_"+ID.str()+".log.txt";
	FracGenLog.open(logName);
	#endif
	Geni[id] = true;
	...
	for (y = 0; y < (int)(ceil(frac.height / threadCount) + 1); y++)
	{
		...
		for (x = 0; x < frac.width; x++)
		{
			...
			for (int st = 0; st < fConfig.setCount; st++)
			{
				...
				#if (__DBG && (__FRACTAL||__COLOR||_THREAD) && __LOG) || __TESTING
				std::string crL = "cr["+std::to_string(st)+"]: ";
				std::string ciL = "ci["+std::to_string(st)+"]: ";
				std::string nL = "n["+std::to_string(st)+"]: ";
				log2txt(FracGenLog,crL,cr[st]);
				log2txt(FracGenLog,ciL,ci[st]);
				log2txt(FracGenLog,nL,n[st]);
				#endif
				...
				#if (__DBG && (__FRACTAL||__COLOR||_THREAD) && __LOG) || __TESTING
				std::string dRedL = "dRed["+std::to_string(st)+"]: ";
				std::string dGreenL = "dGreen["+std::to_string(st)+"]: ";
				std::string dBlueL = "dBlue["+std::to_string(st)+"]: ";
				log2txt(FracGenLog,dRedL,dRed[st]);
				log2txt(FracGenLog,dGreenL,dGreen[st]);
				log2txt(FracGenLog,dBlueL,dBlue[st]);
				#endif
			...
			#if (__DBG && (__FRACTAL||__COLOR||_THREAD) && __LOG) || __TESTING
			std::string ThrdRGBL = "Thread_RGB["+ID.str()+"]["+std::to_string(y)+"]["+std::to_string(x)+"]: ";
			log2txt(FracGenLog,ThrdRGBL,Thread_RGB[id][y][x]);
			#endif
			if (x != 0 && y != 0)
			{
...
void AssemableImage()
{
	#if __DBG && (__COLOR||__OUTPUT) && __LOG
	std::ofstream ImgAssmblLog;
	ImgAssmblLog.open("ImgAssmbl.log.txt");
	#endif
	ImageMaking = true;
	slp(10);
...
void ControlValueInit()
{
	...
	if (threadCount > ncpu)
	{
		...
		threadCount = ncpu;
	}else if(threadCount == ncpu) {
		std::cout << "thread count'" << threadCount << "' is set to the max your cpu can handle'" << ncpu << "' at a hardware level." << std::endl;
	}else{
		std::cout << "thread count'" << threadCount << "' was below your cpu's thread limit'" << ncpu << "'." << std::endl;
	}
...
{
	#if __DBG && (__FRACTAL||__COLOR) && __RANDOM  && __LOG
	std::ofstream iRandNumLog;
	iRandNumLog.open("iRandNum.log.txt");
	#endif
...
{
	#if __DBG && (__FRACTAL||__COLOR) && __RANDOM  && __LOG
	std::ofstream fRandNumLog;
	fRandNumLog.open("fRandNum.log.txt");
	#endif
	srand((unsigned)randSeed);
...
{
	#if __DBG && (__FRACTAL||__COLOR) && __RANDOM  && __LOG
	std::ofstream dRandNumLog;
	dRandNumLog.open("dRandNum.log.txt");
	#endif
	srand((unsigned)randSeed);
